...
from .metrics import *
from .dataloader import FCLmserDataset
from .model import FCLmser
...
def train(args):
    ...
    train_set = FCLmserDataset(args.train_data, 'monuseg', batchsize=args.batch_size, steps=args.steps, transforms=transforms)
    test_set = FCLmserDataset(args.valid_data, args.valid_dataset)
    ...
    # create model
    model = FCLmser(iterations=args.iter,
                   num_classes=args.num_class,
                   num_layers=4,
                   multiplier=args.multiplier,
                   integrate=args.integrate,
                   alpha=args.alpha,
                   layers=args.layers).to(device).float()
    ...
    for epoch in range(args.epochs):
        ...
        tdice.append(tot_dice/args.steps)
        ...
        vdice.append(val_dice/len(test_loader))

    print('\nTraining fininshed!')
    plt.plot(epochnum, tdice, 'b', epochnum, vdice, 'r')
    plt.savefig("checkpoints/"+args.exp+"/"+"%s.jpg"%(args.exp))
...
def evaluate(args):
    ...
    with torch.no_grad():
        for step, (x, y) in enumerate(tqdm(test_loader)):
            ...
            if not os.path.exists("checkpoints/"+args.exp+"/outputs/gt"):
                os.mkdir("checkpoints/"+args.exp+"/outputs/gt")
            y_mask = (y > 0.5)[0][0]
            savename = 'checkpoints/%s/outputs/gt/%s.png'%(args.exp, step)
            plt.imsave(savename, y_mask, cmap='gray')
            
            if not os.path.exists("checkpoints/"+args.exp+"/outputs/result"):
                os.mkdir("checkpoints/"+args.exp+"/outputs/result")
            x_mask = (x > 0.9)[0][0]
            savename = 'checkpoints/%s/outputs/result/%s.png'%(args.exp, step)
            plt.imsave(savename, x_mask, cmap='gray')
